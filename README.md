...
The current site build also includes scope-aware search, exposure/category filters, visual lenses for exposure vs confidence, browser-local saved views, a local watchlist for tracked nodes with per-node notes and markdown report export, clickable insight cards, a signal board for cross-cutting risk and opportunity patterns, an `Analysis Set` workflow for multi-node briefs, a persistent details panel, auto-generated briefing notes for the current scope, focused node, or active analysis set, pinned comparison slots with directional metric deltas, a `Report Pack` export surface that combines the active brief, compare state, and watchlist context, scoped ranking tables, sortable visible-node tables, one-click CSV/JSON export for the current scope, shareable deep links that restore the active view state, responsive mobile-friendly layouts, and clearer confidence/provenance readouts so the static payload is easier to inspect without leaving the treemap view.
